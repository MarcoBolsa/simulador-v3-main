// ARQUIVO: /src/context/SimuladorCalculadora.js (VERSÃO 3.27.3 - MOTOR DE CÁLCULO ISOLADO)
...
/**
 * Função pura que executa todo o motor de cálculo V15.
 * @param {object} form - O objeto de formulário com todos os inputs.
 * @returns {object} O objeto simulationResult (success: boolean, data: object).
 */
export const runSimulationCalculation = (form) => {
    try {
      ...
      const dados = {
        ...
        tipoPlano: form.tipoPlano.toLowerCase(), 
        ...
        baseLance: form.baseDoLance.replace(' ', '_'), 
      ...
      return {
        success: true,
        data: {
          // Mantendo valorCreditoFinal para uso no useEffect
  ...
  useEffect(() => {
    ...
    if (form.baseDoLance === 'Crédito Final' && simulationResult.success) {
      // Usa creditoFinal calculado no simulationResult
