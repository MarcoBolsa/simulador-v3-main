...
  
  // ✅ DADOS DE PRODUÇÃO (230k - Conforme solicitado)
...
  	 	baseLance, upgrade, valorUpgrade, tipoPlano, percentualSeguro, // <- CORRIGIDO
...
  	 	valorCredito, prazoContratado, taxaAdmTotal, tipoPlano, // <- CORRIGIDO
  	 	tipoParcela, calcularSeguro, form.percentualSeguro
  	  );
      
  	  let parcelaPreAtual = pPre;
  	  if (tipoPlano === 'degrau' && mesContemplacao > Math.ceil(prazoContratado / 2)) { // <- CORRIGIDO
  	 	parcelaPreAtual = pPreFinal;
  	  }

  	  let pDetalhePre = `1-${prazoContratado}: ${formatCurrency(pPre)}`;
  	  if (tipoPlano === 'degrau' && pPreFinal !== null) { // <- CORRIGIDO
...
  	 	if (tipoPlano === "degrau" && pIndex >= numParcelasMetade) { // <- CORRIGIDO
...
  	 	  creditoContratado: valorCredito,
          creditoFinal: valorCreditoFinal, // Exporta o crédito final para o useEffect
...
  	 	  parcelaPos: { 
  	 	 	valor: (tipoPlano === 'degrau' && (mesContemplacao-1) >= pontoDeViradaOriginal) ? Math.max(0, novaParcela2) : Math.max(0, novaParcela1), // <- CORRIGIDO
...
  	  const novoFR = regra.fundoReserva ? regra.fundoReserva.toString() : '0';
      setForm(prev => ({
        ...prev,
        taxaAdm: novaTaxa,
    	  fundoReserva: novoFR, 
