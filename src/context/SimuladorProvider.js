/*
 * ARQUIVO: /src/context/SimuladorProvider.js (VERSÃO 3.13 - MOTOR V15)
 *
 * O QUE MUDOU:
 * - O motor de cálculo (runSimulation) foi COMPLETAMENTE REESCRITO.
 * - Ele agora segue 100% o fluxo e as fórmulas do MEMORIAL DE CÁLCULO V15 .
 * - O formulário agora é PRÉ-PREENCHIDO com os dados de teste das imagens
 * para validação imediata.
 * - Corrigidos todos os bugs de referência ('valorCredito is not defined', etc.).
 */
...
export const SimuladorProvider = ({ children }) => {
  ...
  // Lógica de Prazo e Furo V15 [cite: 1018-1020]
  ...
  // Helper 3.1: Calcular Parcela Base [cite: 955-1012]
  ...
  const simulationResult = useMemo(() => {
    try {
      // 1. MAPEAMENTO: Interface (form) -> DadosEntrada (V15) [cite: 919-943]
      const dados = {
      ...
      const {
          ...
          baseLance, upgrade, upgradeValor, planoPagamento, percentualSeguro,
      ...
      // 3.2.1 Crédito Reajustado [cite: 1016-1020]
      ...
      // 3.2.2 Crédito Final (com Upgrade) [cite: 1021-1030]
      ...
      const { parcela: pLance1, parcelaFinal: pLance2 } = calcularParcelaBase(
        creditoBaseLance, prazoContratado, taxaAdmTotal, planoPagamento, 
      ...
      for (let i = 0; i < furoN; i++) {
        ...
        if (planoPagamento === "degrau" && pIndex >= numParcelasMetade) {
      ...
      // 3.2.4 Cálculo do Lance Total (R$) [cite: 1037-1047]
      let lanceTotalR = 0;
      if (usarLanceValor) {
      ...
      // 3.2.6 Lance Embutido e Crédito Líquido [cite: 1054-1059]
      ...
      // 3.2.7 Recurso Próprio (Lance Bolso) [cite: 1060-1064]
      ...
      // 3.2.8 Saldo do Lance (R$) [cite: 1065-1069]
      let saldoLanceR = 0;
      let saldoLanceN = 0;
      if (!usarLanceValor) {
      ...
      // --- 3.3 CÁLCULO PARCELA PÓS --- [cite: 1070-1113]
      
      // 3.3.1 Prazo Final Restante [cite: 1073-1080]
      ...
      // 3.3.2 Diferença Acumulada [cite: 1081-1090]
      ...
      if (tipoParcela !== "Integral") {
        for (let mes = 1; mes < mesContemplacao; mes++) {
          ...
          // **CORREÇÃO V3.10**: Usar getParcelas (o nome da nossa função)
          const pIntegralBase = getParcelas(creditoDoMes, prazoContratado, taxaAdmTotal, planoPagamento, "Integral", false, '0').parcela;
          const pReduzidaBase = getParcelas(creditoDoMes, prazoContratado, taxaAdmTotal, planoPagamento, tipoParcela, false, '0').parcela;
      ...
      // 3.3.5 Desconto do Saldo do Lance [cite: 1100-1102]
      const descontoMensal = (estrategiaPos === "Reduzir_Valor" && prazoFinal > 0) ? saldoLanceR / prazoFinal : 0;
      
      // 3.3.6 & 3.3.7 Fórmula Final Pós [cite: 1103-1113]
      let novaParcela1 = 0;
      let novaParcela2 = 0;
      
      if (baseLance === "Credito_Final" || baseLance === "Parcela_Integral" || baseLance === "Parcela_Reduzida") {
        // Fórmula 3.3.6 [cite: 1103-1107]
        // **CORREÇÃO V3.10**: Usar getParcelas
        const finalBase = getParcelas(valorCreditoFinal, prazoContratado, taxaAdmTotal, planoPagamento, "Integral", calcularSeguro, form.percentualSeguro);
      ...
      } else { // "Crédito_Inicial" [cite: 1108-1113]
        // Fórmula 3.3.7
        ...
        // **CORREÇÃO V3.10**: Usar getParcelas
        const preBase = getParcelas(valorCredito, prazoContratado, taxaAdmTotal, planoPagamento, tipoParcela, false, '0');
      ...
      return {
        ...
        data: {
          ...
          parcelaPos: { 
            valor: (planoPagamento === 'degrau' && (mesContemplacao-1) >= pontoDeViradaOriginal) ? Math.max(0, novaParcela2) : Math.max(0, novaParcela1),
